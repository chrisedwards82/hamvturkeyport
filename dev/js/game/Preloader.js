...
(function(){
	...
	p.intro = function(callback){
		...
		createjs.Tween.get(this.thermometer).wait(delay+=300).to({scaleX:1,scaleY:1}, 800,createjs.Ease.elasticOut);
		createjs.Tween.get(this.arrow).wait(delay+=800).to({alpha:1},300).call(callback);
	...
	//	console.log(event);
		...
		if(!isNaN(targ)){
			createjs.Tween.get(this.arrow,{override:true}).to({rotation:targ},1500,createjs.Ease.elasticInOut);
			//console.log(targ);
		}
	}
	...
	p.outro = function(){
		...
		createjs.Tween.get(this.thermometer).wait(100).to(
			{scaleX:0,scaleY:0}, 500,createjs.Ease.backIn).call(createjs.proxy(this.outroed,this));
	}
	p.outroed = function(){
